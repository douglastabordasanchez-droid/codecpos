                ...
                {contratado.map((c: any) => (
                  <tr key={c.id} className="border-b border-slate-800/60 last:border-0">
                    <td className="px-4 py-3">
                      {c.clientes_pos ? <Link to={`/clientes/${c.clientes_pos.id}`} className="hover:text-amber-400">{c.clientes_pos.nombre_negocio}</Link> : '—'}
                    </td>
              ...
              <SectionCard key={s.id}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    {s.clientes_pos && <p className="text-xs text-slate-400">{s.clientes_pos.nombre_negocio} · {formatoFechaHora(s.created_at)}</p>}
